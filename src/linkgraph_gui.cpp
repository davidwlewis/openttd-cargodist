...
void LinkGraphOverlay::DrawContent(Point pta, Point ptb, const LinkProperties &cargo) const
{
	...
	int offset = CeilDiv(this->scale, 2);
	int offset_y = (pta.x < ptb.x ? 1 : -1) * offset;
	int offset_x = (pta.y > ptb.y ? 1 : -1) * offset;
	...
	GfxDrawLine(pta.x, pta.y, ptb.x, ptb.y, _colour_gradient[COLOUR_GREY][1], this->scale > 1 ? 3 : 1);
}
